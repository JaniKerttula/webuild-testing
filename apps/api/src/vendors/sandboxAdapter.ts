...
type IssueCredentialResponse = {
  ...
  credential_offer?: string | Record<string, unknown>;
  userPin?: string;
  user_pin?: string;
  txCode?: string;
  tx_code?: string;
...
type IgrantConfig = {
  ...
  authScheme: string;
  apiKey: string;
  personalCredentialDefinitionId: string;
  poaCredentialDefinitionId: string;
  euccCredentialDefinitionId: string;
  vatCredentialDefinitionId: string;
};
...
type ListVerifierGlobalConfigurationResponse = {
  verifierGlobalConfiguration?: Array<Record<string, unknown>> | Record<string, unknown>;
  verifierGlobalConfigurations?: Array<Record<string, unknown>> | Record<string, unknown>;
};
...
async function readSharedEnvFile(): Promise<Record<string, string>> {
...
function createNotImplementedError(message: string): NormalizedError {
  return {
...
function createFailureResult<T>(message: string, error?: NormalizedError): AdapterOperationResult<T> {
  return {
    status: 'failed',
...
function createConfigurationError(message: string): AdapterOperationResult<never> {
...
function createHttpError(status: number, detail: string): AdapterOperationResult<never> {
...
function isConfigError(value: IgrantConfig | AdapterOperationResult<never>): value is AdapterOperationResult<never> {
  return 'error' in value;
}

async function readConfig(): Promise<IgrantConfig | AdapterOperationResult<never>> {
  const sharedEnv = await readSharedEnvFile();
  const readEnvValue = (key: string): string | undefined => process.env[key] ?? sharedEnv[key];
...
function getCredentialExchangeId(payload: IssueCredentialResponse): string | undefined {
...
function getCredentialOfferValue(payload: IssueCredentialResponse): string | Record<string, unknown> | undefined {
...
function getInlineCredentialOfferValue(payload: IssueCredentialResponse): string | Record<string, unknown> | undefined {
...
function getCredentialOfferUri(payload: IssueCredentialResponse): string | undefined {
...
function getQrCodeValue(payload: IssueCredentialResponse, offerUri: string): string {
...
function getReferencedOfferUri(offerUri: string): string | undefined {
  try {
...
function getCredentialUserPin(payload: IssueCredentialResponse, config: IgrantConfig): string | undefined {
...
function extractStatus(payload: IssuanceHistoryResponse): string {
  return String(
    payload.status
...
type VatIssuanceHistoryStatus = typeof vatIssuanceHistoryStatus[keyof typeof vatIssuanceHistoryStatus];

function normalizeVatIssuanceHistoryStatus(status: string): VatIssuanceHistoryStatus {
...
function normalizePoaRecordFromClaims(claims: Record<string, unknown>): PoaRecord | undefined {
...
function normalizeEuccRecordFromClaims(claims: Record<string, unknown>): EuccRecord | undefined {
...
function parseSdJwtClaims(sdJwt: string): Record<string, unknown> | undefined {
...
function getBooleanClaim(claims: Record<string, unknown>, key: string): boolean | undefined {
  const value = claims[key];
...
function toPidRecordFromClaims(claims: Record<string, unknown>, fixture: PidFixture): PidRecord | undefined {
...
function extractPidRecordFromUnknown(value: unknown, fixture: PidFixture): PidRecord | undefined {
  if (typeof value === 'string') {
...
function extractPidRecordFromVerificationHistory(history: VerificationHistoryResponse, fixture: PidFixture): PidRecord | undefined {
...
function extractRecordFromUnknown<T>(
  value: unknown,
  directExtractor: (claims: Record<string, unknown>) => T | undefined,
): T | undefined {
  if (typeof value === 'string') {
    const parsedClaims = parseSdJwtClaims(value);
...
function extractPoaRecordFromVerificationHistory(history: VerificationHistoryResponse): PoaRecord | undefined {
  const candidates: unknown[] = [];
...
function extractEuccRecordFromVerificationHistory(history: VerificationHistoryResponse): EuccRecord | undefined {
  const candidates: unknown[] = [];
...
function normalizeVatPending(attestation: VatAttestationRecord, exchangeId: string): VatIssuanceResult {
  return {
...
function normalizeVatIssued(attestation: VatAttestationRecord, exchangeId?: string): VatIssuanceResult {
  return {
    vatId: attestation.vatId,
    administrativeUnitName: attestation.administrativeUnitName,
    administrativeUnitType: attestation.administrativeUnitType,
...
function createVatWalletOfferSummary(
  attestation: VatAttestationRecord,
  offerUri: string,
  qrCodeValue: string,
  exchangeId: string | undefined,
...
function buildVatCredentialClaims(attestation: VatAttestationRecord, administrativeUnitAddress?: Record<string, unknown>, validityPeriod?: Array<Record<string, unknown>>, economicActivityType?: Array<Record<string, unknown>>, validityAreaLimitation?: unknown[], attestationIssuerName?: string): Record<string, unknown> {
  return {
...
function buildVatJwtVcClaims(
  attestation: VatAttestationRecord,
  administrativeUnitAddress?: Record<string, unknown>,
  validityPeriod?: Array<Record<string, unknown>>,
  economicActivityType?: Array<Record<string, unknown>>,
  validityAreaLimitation?: unknown[],
  attestationIssuerName?: string,
): Record<string, unknown> {
  return {
    credentialSubject: buildVatCredentialClaims(
      attestation,
      administrativeUnitAddress,
      validityPeriod,
      economicActivityType,
      validityAreaLimitation,
      attestationIssuerName,
    ),
  };
}

function buildVatJwtVcCredentialSubject(
  attestation: VatAttestationRecord,
  administrativeUnitAddress?: Record<string, unknown>,
  validityPeriod?: Array<Record<string, unknown>>,
  economicActivityType?: Array<Record<string, unknown>>,
...
async function issueSeedCredential(
  config: IgrantConfig,
  credentialType: WalletCredentialType,
  ...
  const response = await requestIgrant<IssueCredentialResponse>(config, '/v2/config/digital-wallet/openid/sdjwt/credential/issue', {
    method: 'POST',
    body: JSON.stringify({
      issuanceMode: 'InTime',
  ...
  return {
    status: 'succeeded',
    data: createWalletCredentialSummary(
      credentialType,
      holderName,
      issuerName,
      seededAt,
      offerUri,
      qrCodeValue,
      getCredentialExchangeId(response),
      getCredentialUserPin(response, config),
    ),
    message: `Created iGrant ${credentialType.toUpperCase()} OID4VCI offer for wallet pickup.`,
  };
}

async function resolveAndIssueSeedCredential(
  config: IgrantConfig,
  credentialType: WalletCredentialType,
  credentialDefinitionId: string,
  claims: Record<string, unknown>,
...
async function requestIgrant<T>(config: IgrantConfig, path: string, init: RequestInit): Promise<T | AdapterOperationResult<never>> {
...
type VerificationHistoryLookupResult =
  | {
    status: 'error';
    error: AdapterOperationResult<never>;
  }
  | {
    status: 'empty';
  }
...
async function requestVerificationHistory(
  config: IgrantConfig,
  exchangeId: string,
): Promise<VerificationHistoryLookupResult> {
  ...
  const payload = parseJson<VerificationRequestResponse>(text);
...
function isAdapterFailure<T>(value: T | AdapterOperationResult<never>): value is AdapterOperationResult<never> {
  return typeof value === 'object' && value !== null && 'status' in value && 'message' in value;
}

function getCredentialDefinitionEntry(response: CredentialDefinitionResponse) {
  return response.credentialDefinition?.credentialDefinitions?.[0];
}

async function readCredentialDefinition(
  config: IgrantConfig,
  credentialDefinitionId: string,
): Promise<CredentialDefinitionResponse | AdapterOperationResult<never>> {
  return requestIgrant<CredentialDefinitionResponse>(
    config,
...
async function resolveCredentialEntryId(
  config: IgrantConfig,
  credentialDefinitionId: string,
): Promise<string | AdapterOperationResult<never>> {
...
async function ensureVerifierGlobalConfiguration(config: IgrantConfig): Promise<AdapterOperationResult<never> | undefined> {
  const response = await requestIgrant<ListVerifierGlobalConfigurationResponse>(
    config,
  ...
  const createResponse = await requestIgrant<Record<string, unknown>>(
    config,
...
function buildPresentationDefinitionBody(
  credentialDefinition: CredentialDefinitionResponse,
  requestLabel: string,
...
function getPresentationDefinitionId(payload: PresentationDefinitionResponse): string | undefined {
  return payload.presentationDefinition?.presentationDefinitionId ?? payload.presentationDefinition?.id;
}

function getVerificationHistory(payload: VerificationRequestResponse): VerificationHistoryResponse | undefined {
...
function getVerificationRequestUri(payload: VerificationRequestResponse): string | undefined {
...
function getOpenId4VpUri(payload: VerificationRequestResponse, requestUri: string | undefined): string | undefined {
  const history = getVerificationHistory(payload);
...
function getPresentationExchangeId(payload: VerificationRequestResponse): string | undefined {
  const history = getVerificationHistory(payload);
  return history?.presentationExchangeId ?? history?.id;
}

function isVerificationReadyForExtraction(history: VerificationHistoryResponse): boolean {
...
export function createIgrantSandboxAdapter(): VendorAdapter {
  return {
    definition,
    async seedWalletCredential(
      ...
      if (walletRole === 'personal' && credentialType === 'pid') {
        ...
        return resolveAndIssueSeedCredential(
          config,
          'pid',
          config.personalCredentialDefinitionId,
          {
      ...
      if (walletRole === 'personal' && credentialType === 'poa') {
        ...
        return resolveAndIssueSeedCredential(
          config,
          'poa',
          config.poaCredentialDefinitionId,
          fixtures.poa as unknown as Record<string, unknown>,
      ...
      if (walletRole === 'company' && credentialType === 'eucc') {
        ...
        return resolveAndIssueSeedCredential(
          config,
          'eucc',
          config.euccCredentialDefinitionId,
          fixtures.eucc as unknown as Record<string, unknown>,
    ...
    async requestPid(_session: OrchestrationSession, options?: AdapterRequestOptions) {
      if (options?.simulationMode === 'failure') {
        return createFailureResult<PidStepData>('Simulated iGrant PID verification failure.');
      }
      ...
      const presentationDefinitionBody = buildPresentationDefinitionBody(
        credentialDefinitionResponse,
        `We Build Testing PID verification ${_session.sessionId} ${Date.now()}`,
        'pid',
      );
      ...
      const presentationDefinitionResponse = await requestIgrant<PresentationDefinitionResponse>(
        config,
      ...
      const response = await requestIgrant<VerificationRequestResponse>(config, '/v3/config/digital-wallet/openid/sdjwt/verification/send', {
        method: 'POST',
        body: JSON.stringify({
          presentationDefinitionId,
    ...
    async requestPoa(_session: OrchestrationSession, options?: AdapterRequestOptions) {
      if (options?.simulationMode === 'failure') {
        return createFailureResult<PoaStepData>('Simulated iGrant PoA verification failure.');
      }
      ...
      const presentationDefinitionBody = buildPresentationDefinitionBody(
        credentialDefinitionResponse,
        `We Build Testing PoA verification ${_session.sessionId} ${Date.now()}`,
        'poa',
      );
    ...
    async requestEucc(_session: OrchestrationSession, options?: AdapterRequestOptions) {
      if (options?.simulationMode === 'failure') {
        return createFailureResult<EuccStepData>('Simulated iGrant EUCC verification failure.');
      }
      ...
      const presentationDefinitionBody = buildPresentationDefinitionBody(
        credentialDefinitionResponse,
        `We Build Testing EUCC verification ${_session.sessionId} ${Date.now()}`,
        'eucc',
      );
    ...
    async assembleReview(session: OrchestrationSession, _options?: AdapterRequestOptions) {
    ...
    async submitVatIssuance(session: OrchestrationSession, _options?: AdapterRequestOptions) {
      const config = await readConfig();
          ...
          credentials: [
            {
              ...
              credentialSubject: buildVatJwtVcCredentialSubject(
                attestation,
                matchedFixture.Administrative_Unit_Address,
                matchedFixture.Validity_Period,
                matchedFixture.Economic_Activity_Type,
                matchedFixture.Validity_Area_Limitation,
                matchedFixture.Issuer.Attestation_issuing_Organisation,
              ),
            },
          ],
          userPin: '',
      ...
      const walletCredential = offerUri
        ? createVatWalletOfferSummary(
          attestation,
          offerUri,
          getQrCodeValue(response, offerUri),
          exchangeId,
          getCredentialUserPin(response, config),
        )
        : undefined;
    ...
    async readIssuanceStatus(session: OrchestrationSession, _options?: AdapterRequestOptions) {
      ...
      const response = await requestIgrant<IssuanceHistoryResponse>(
        config,
      ...
      if (
        normalizedStatus === vatIssuanceHistoryStatus.credentialDeleted
